...
class SourcePluginLoader extends PluginLoader {
  ...
  getPluginManifest(file) {
    if (FileSystem.lstatSync(file).isDirectory() && FileSystem.existsSync(file + "/manifest.json")) {
      let data = FileSystem.readFileSync(file + "/manifest.json", {
        encoding: "utf-8"
      });
  ...
  enablePlugin(plugin) {
    if (plugin instanceof PluginBase && !plugin.isEnabled()) {
      ...
      plugin.setEnabled(true); //todo: event stuff: call PluginEnableEvent
    }
  }

  disablePlugin(plugin) {
    if (plugin instanceof PluginBase && plugin.isEnabled()) {
      this.server.getLogger().info("Disabling " + plugin.getFullName()); //todo: event stuff: call PluginDisableEvent
