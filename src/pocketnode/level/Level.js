...
class Level {
  constructor(server, name, provider) {
    this.initVars(); //this._blockStates = BlockFactory; //TODO

    this._levelId = this._levelIdCounter++; //this._blockMetadata = new BlockMetadataStore(this);
  ...
  actuallyDoTick(currentTick) {} //this.getServer().getSessionAdapter().test();
  ...
  isInWorld(x, y, z) {
    return x <= INT32_MAX && x >= INT32_MIN && y < this._worldHeight && y >= 0 && z <= INT32_MAX && z >= INT32_MIN;
  }
  /**
   * @param x {Number}
   * @param z {Number}
