...
class SubChunk extends SubChunkInterface {
  ...
  isEmpty(checkLight = true) {
    return this._blockIds.filter(id => id === 0x00).length === 4096 && (!checkLight || this._blockLight.filter(id => id === 0x00).length === 2048 && this._skyLight.filter(id => id === 0xff).length === 2048);
  }
