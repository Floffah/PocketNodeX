class NBTTest {
  ...
  constructor() {
    ...
    this.initVars();
    var parsedObject = {}; //console.log(decToHex(this.tags.TAG_STRING.getId()));

    var fs = require("fs");

    var buffer = fs.readFileSync(__dirname + "/hello_world.nbt"); //buffer = zlib.gunzipSync(buffer);

    buffer = buffer.toString('hex');
    buffer = splitString(buffer, 2); //console.log(buffer)

    console.log(JSON.stringify(this.parseTag(buffer, {}))); //console.log(JSON.stringify(squashObject(this.returnNBT), false, 4));
    ...
    for (var t = 0; t < Object.keys(this.tags).length; t++) {
      ...
      if (forTag.getId() === type) {
        ...
        if (!tag.isEnd()) {
          ...
          buffer = tag.setName(buffer, length);
          var name = tag.getName();
          console.log(type, length, name, tag.isCompound()); //console.log(buffer)
          ...
          console.log(buffer); //var postPayload = tag.setPayload(buffer)
          ////ReturnObject[hexToAscii(name)] = postPayload.payload;
          //buffer = postPayload.buffer;

          if (buffer.length != 0) {
            var tag = this.parseTag(buffer, parentObject);
            console.log(JSON.stringify(tag)); //ReturnObject[hexToAscii(tag.name)] = tag.return;
          }
        }
      }
    }
