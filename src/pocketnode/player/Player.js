...
const AddActorPacket = require("../network/mcpe/protocol/AddActorPacket"); // const SerializedImage = require("../utils/SerializedImage");
...
class Player extends Human {
  constructor(sourceInterface, server, clientId, ip, port) {
    ...
    this.level = server.getDefaultLevel(); //this.namedtag = new CompoundTag();
  ...
  handleLogin(packet) {
    ...
    this._uuid = UUID.fromString(packet.clientUUID); // this._rawUUID = this._uuid.toBinary(); to fix

    let animations = [];
    packet.clientData['AnimatedImageData'].forEach(animation => {
      animations.push(new SkinAnimation(new SkinImage(animation['ImageHeight'], animation['ImageWidth'], Base64.decode(animation['Image'], true)), animation['Type'], animation['Frames']));
    });
    let skinData = new SkinData(packet.clientData['SkinId'], Base64.decode(packet.clientData['SkinResourcePatch'] || "", true), new SkinImage(packet.clientData['SkinImageHeight'], packet.clientData['SkinImageWidth'], Base64.decode(packet.clientData['SkinData'], true)), animations, new SkinImage(packet.clientData['CapeImageHeight'], packet.clientData['CapeImageWidth'], Base64.decode(packet.clientData['CapeData'] || "", true)), Base64.decode(packet.clientData['SkinGeometryData'] || "", true), Base64.decode(packet.clientData['SkinAnimationData'] || "", true), packet.clientData['PremiumSkin'] || false, packet.clientData['PersonaSkin'] || false, packet.clientData['CapeOnClassicSkin'] || false, packet.clientData['CapeId'] || "");
    ...
    this.setSkin(skin); // let ev = new PlayerPreLoginEvent(this, "Test");
  ...
  doFirstSpawn() {
    this.spawned = true; //this.sendPlayStatus(PlayStatusPacket.PLAYER_SPAWN);
  ...
  close(message, reason = "generic reason", notify = true) {
    if (this.isConnected() && !this.closed) {
      try {
        ...
        this._sessionAdapter = null; //unsub from perms?
        //stopsleep

        if (this.joined) {
          try {//save player data
          } catch (e) {
        ...
        this.joined = false; //if valid do chuck stuff

        if (this.loggedIn) {
          this.server.onPlayerLogout(this); //can see etc
        }
  ...
  completeLoginSequence() {
    ...
    let pk = new StartGamePacket(); // pk.entityUniqueId = this.id;
    // pk.entityRuntimeId = this.id;
    ...
    this.sendDataPacket(new BiomeDefinitionListPacket()); // this.level.sendTime(this);
    // this.sendPlayStatus(PlayStatusPacket.PLAYER_SPAWN, true);

    this.sendAttributes(true);
    this.server.getLogger().info([TextFormat.AQUA + this.getName() + TextFormat.WHITE + " (" + this._ip + ":" + this._port + ")", "is attempting to join"].join(" ")); //this.sendCommandData();
    ...
    this.server.onPlayerCompleteLoginSequence(this); // this.server.sendFullPlayerListData(this);
    ...
    this.server.getCommandMap().getCommands().forEach(command => {
      for (let name in command) {
        if (command.hasOwnProperty(name)) {
          ...
          let data = new CommandData(); //TODO: commands containing uppercase letters in the name crash 1.9.0 client
  ...
  handleAdventureSettings(packet) {//TODO
  }
  ...
  handleResourcePackClientResponse(packet) {
    ...
    let pk, manager; // console.log("Status:", ResourcePackClientResponsePacket.STATUS(packet.status));
    ...
    switch (packet.status) {
      ...
      case ResourcePackClientResponsePacket.STATUS_HAVE_ALL_PACKS:
        ...
        pk.mustAccept = false; // manager = this.server.getResourcePackManager();
  ...
  handleEntityEvent(packet) {
    ...
    switch (packet.event) {
      case ActorEventPacket.EATING_ITEM:
        ...
        this.dataPacket(packet); //TODO this.server.broadcastPacket();
  ...
  processMovement() {
    let newPos = this.newPosition; //let distanceSquared = newPos.distanceSquared(this);
  ...
  onUpdate(currentTick) {
    ...
    this.lastUpdate = currentTick; // this.sendAttributes();

    this.processMovement(); //this.processChunkRrquest();
  ...
  sendAttributes(sendAll = false) {
    ...
    pk.entityRuntimeId = this.id;
    pk.entries = [new Attribute(0, "minecraft:absorption", 0.00, 340282346638528859811704183484516925440.00, 0.00, true), new Attribute(1, "minecraft:player.saturation", 0.00, 20.0, 20.0, true), new Attribute(2, "minecraft:player.exhaustion", 0.00, 5.00, 0.0, false), new Attribute(3, "minecraft:knockback_resistance", 0.00, 1.00, 0.00, true), new Attribute(4, "minecraft:health", 0.00, 20.00, 20.00, true), new Attribute(5, "minecraft:movement", 0.00, 340282346638528859811704183484516925440.00, 0.10, true), new Attribute(6, "minecraft:follow_range", 0.00, 2048.00, 16.00, false), new Attribute(7, "minecraft:player.hunger", 0.00, 20.00, 20.00, true), new Attribute(8, "minecraft:attack_damage", 0.00, 340282346638528859811704183484516925440.00, 1.00, false), new Attribute(9, "minecraft:player.level", 0.00, 24791.00, 0.00, true), new Attribute(10, "minecraft:player.experience", 0.00, 1.00, 0.00, true), new Attribute(11, "minecraft:underwater_movement", 0.0, 340282346638528859811704183484516925440.0, 0., true), new Attribute(12, "minecraft:luck", -1024.0, 1024.0, 0.0, true), new Attribute(13, "minecraft:fall_damage", 0.0, 340282346638528859811704183484516925440.0, 1.0, true), new Attribute(14, "minecraft:horse.jump_strength", 0.0, 2.0, 0.7, true), new Attribute(15, "minecraft:zombie.spawn_reinforcements", 0.0, 1.0, 0.0)];
    this.dataPacket(pk);
    /*if (entries.length > 0){
        let pk = new UpdateAttributesPacket();
        pk.entityRuntimeId = this.id;
  ...
  sendPosition(pos, yaw = null, pitch = null, mode = MovePlayerPacket.MODE_NORMAL, targets = null) {
    ...
    let playerYaw = this._yaw; // let playerPitch = (pitch ?? this._pitch);
  ...
  sendSettings() {
    ...
    pk.setFlag(AdventureSettingsPacket.FLYING, this._flying); //TODO Op system.
  ...
  handlePlayerAction(packet) {
    ...
    switch (packet.action) {
      ...
      case PlayerActionPacket.ACTION_START_SPRINT:
        console.log("PlayerActionHandler sprint toggled"); //todo this.toggleSprint(true);
