...
class RakNetAdapter {
  constructor(server) {
    _defineProperty(this, "_identifiersACK", []);

    this.server = server;
    this.raknet = new RakNetServer(server.getPort(), new Logger("RakNet").setDebugging(server._debuggingLevel));
    this.raknet.getServerName().setServerId(server.getServerId()).setMotd(server.getMotd()).setName(server.getName()).setProtocol(server.getProtocol()).setVersion(server.getVersion()).setOnlinePlayers(server.getOnlinePlayerCount()).setMaxPlayers(server.getMaxPlayers()).setGamemode(server.getGamemode() >= 1 ? "Creative" : "Survival"); //todo fix this later
  ...
  putPacket(player, packet, needACK, immediate) {
    if (this.players.hasPlayer(player)) {
      ...
      if (packet instanceof BatchPacket) {
        ...
        this.server.batchPackets([player], [packet], true, immediate); //this.logger.debugExtensive("Sending "+packet.getName()+":", packet.buffer);
