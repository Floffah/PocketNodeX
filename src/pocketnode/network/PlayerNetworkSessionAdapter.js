...
const Async = require("../utils/Async"); // const aasync = require('async'); TODO
...
class PlayerNetworkSessionAdapter {
  /** @type {boolean} */
  constructor(player) {
    _defineProperty(this, "_hasChunks", false);
  ...
  handleDataPacket(packet) {
    ...
    packet.handle(this); // let ev = new DataPacketReceiveEvent(this.player, packet);
  ...
  handleInventoryTransaction(packet) {//TODO
  }
  ...
  handleResourcePackChunkRequest(packet) {
    ...
    let pack = manager.getPackById(packet.packId); //let pack = manager.getPackById(uuid.substr(0), (uuid + "").indexOf("_"));
  ...
  handleRequestChunkRadius(packet) {
    ...
    Async(function () {
      ...
      console.log("done sending chunks"); // TODO
