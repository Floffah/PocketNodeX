...
class LegacySkinAdapter {
  ...
  toSkinData(skin) {
    ...
    return new SkinData(skin.getSkinId(), JSON.stringify({
      'geometry': {
  ...
  fromSkinData(data) {
    if (data.isPersona()) {
      return new Skin('Standard_Custom', Crypto.randomBytes(3).toString('hex') + '\xff'.repeat(2048));
    }
    ...
    if (typeof resourcePatch['geometry']['default'] !== 'undefined' && typeof resourcePatch['geometry']['default'] === 'string') {
