...
class NetworkBinaryStream extends require("../../../binarystream/BinaryStream") {
  ...
  writeUUID(uuid) {
    this.writeLInt(uuid.getPart(1)).writeLInt(uuid.getPart(0)).writeLInt(uuid.getPart(3)).writeLInt(uuid.getPart(2));
    return this;
  }
  ...
  readVector3() {
    return new Vector3(this.readRoundedLFloat(4), this.readRoundedLFloat(4), this.readRoundedLFloat(4));
  }
  ...
  readSkin() {
    ...
    for (let i = 0; i < animationCount; i++) {
      animations.push(new SkinAnimation(skinImage = this.readSkinImage(), animationType = this.readLInt(), animationFrames = this.readLFloat()));
    }
    ...
    let capeId = this.readString(); // let fullSkinId = this.readString();
  ...
  readBlockPosition() {
    return [this.readVarInt(), this.readUnsignedVarInt(), this.readVarInt()];
  }

  writeBlockPosition(x, y, z) {
    this.writeVarInt(x).writeUnsignedVarInt(y).writeVarInt(z);
    return this;
  }

  readSignedBlockPosition(x, y, z) {
    return [this.readVarInt(), this.readVarInt(), this.readVarInt()];
  }
