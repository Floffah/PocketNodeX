...
class ConnectionRequestAccepted extends Packet {
  ...
  initVars() {
    this.address = "";
    this.port = -1;
    this.systemAddresses = [["127.0.0.1", 0, 4]];
    this.sendPingTime = -1;
    this.sendPongTime = -1;
  }

  encodePayload() {
    this.getStream().writeAddress(this.address, this.port, 4).writeShort(0);
    ...
    this.getStream().writeLong(this.sendPingTime).writeLong(this.sendPongTime);
  }
