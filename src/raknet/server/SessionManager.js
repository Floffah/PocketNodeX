...
class SessionManager {
  ...
  start() {
    this._socket.getSocket().on("message", (msg, rinfo) => {
      ...
      let packetId = stream.getBuffer()[0]; //this.logger.debug("Received", packetId, "with length of", msg.length, "from", rinfo.address + ":" + rinfo.port);
  ...
  sendPacket(packet, address, port) {
    ...
    this.getLogger().debug("Sent " + packet.constructor.name + "(" + packet.stream.buffer.toString("hex") + ") to " + address + ":" + port); // console.log("Sent "+packet.constructor.name+"("+packet.stream.buffer.toString("hex")+") to "+address+":"+port);
  }
  ...
  handle(packetId, stream, ip, port) {
    let session = this.getSession(ip, port); //console.log("got packet!", stream);
