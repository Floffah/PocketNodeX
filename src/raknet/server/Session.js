...
class Session {
  ...
  static get STATE_DISCONNECTING() {
    return 2;
  }

  static get STATE_DISCONNECTED() {
    return 3;
  }
  ...
  update(time) {
    ...
    if (this.state === Session.STATE_DISCONNECTING && this.ACKQueue.isEmpty() && this.NACKQueue.isEmpty() && this.packetsToSend.length === 0 && this.recoveryQueue.isEmpty() && this.disconnectionTime + 10 < time) {
      this.close();
      return;
    }
  ...
  handlePacket(packet) {
    ...
    if (packet instanceof Datagram || packet instanceof ACK || packet instanceof NACK) {// this.sessionManager.getLogger().debug("Got " + protocol.constructor.name + "(" + protocol.stream.buffer.toString("hex") + ") from " + this);
    }
  ...
  handleEncapsulatedPacket(packet) {
    if (!(packet instanceof EncapsulatedPacket)) throw new TypeError("Expecting EncapsulatedPacket, got " + (packet.constructor.name ? packet.constructor.name : packet)); //this.sessionManager.getLogger().debug("Handling EncapsulatedPacket("+protocol.getBuffer().toString("hex")+")["+protocol.getBuffer().length+"] from "+this);
    ...
    switch (id) {
      ...
      case NewIncomingConnection.getId():
        ...
        if (true || dpk.port === this.sessionManager.getPort()) {
          //todo: if port checking
          this.setConnected();
  ...
  queueConnectedPacket(packet, reliability, orderChannel, flags = RakNet.PRIORITY_NORMAL) {
    ...
    pk.stream = new BinaryStream(packet.getBuffer()); //this.sessionManager.getLogger().debug("Queuing "+protocol.constructor.name+"("+protocol.getBuffer().toString("hex")+")");
